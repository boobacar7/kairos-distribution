...
describe('storefront shell', () => {
  ...
  it('shows the real cart badge count', () => {
    render(<CartIcon count={3} />);
    expect(screen.getByRole('link', { name: t('cart.count', { count: 3 }) })).toBeInTheDocument();
    expect(screen.getByText('3')).toBeInTheDocument();
  });

  it('shows the cart badge on the mobile panier tab', () => {
    render(<MobileBottomNav cartCount={3} />);
    expect(screen.getByRole('link', { name: t('cart.count', { count: 3 }) })).toBeInTheDocument();
    expect(screen.getByText('3')).toBeInTheDocument();
  });
});
